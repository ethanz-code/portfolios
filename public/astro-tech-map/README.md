...
- 加项目卡片亮暗图：`src/content/projects/*.md` 或 `*.mdx` 的 `heroImageLight` / `heroImageDark`
- 加项目详情截图：在项目 `.mdx` 里从 `src/assets/projects/` 直接导入并传给 `ArticleThemeImage`
...
> 用 Astro 做静态站点，用 Content Collections 管理文章和项目，用 Layout 统一页面结构，用 Tailwind/DaisyUI 管理视觉系统，用 `src/assets` 直接导入让组件图片走 Astro Image 优化。
