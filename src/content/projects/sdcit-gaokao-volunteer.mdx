...
当时的需求文档把系统分成管理员账号和普通账号。管理员负责后台维护，普通用户负责按高考分数、省份、批次查询推荐结果。
...
后台部分除了学校和分数线数据，还包含一套完整的基础管理能力：

- 用户管理：新增、编辑、导入、导出、重置密码、分配权限。
- 角色和菜单管理：维护角色、菜单、状态和权限分配。
- 组织管理：维护部门、岗位等基础组织数据。
- 日志和公告：记录登录日志、操作日志，维护系统通知公告。
- 系统监控：查看在线用户、服务状态、缓存状态，并支持强制用户退出。

这部分更像一个完整后台，而不是只做几张学校表单。我的工作重点是把这些后台能力接到前端页面里，再把志愿推荐相关的数据维护流程串起来，让运营人员能持续更新数据。
...
项目仓库统计里，我个人提交 89 次。因为项目不是个人产品，也不是公开仓库，这里只展示提交次数统计，不公开源码、客户信息和更细的业务页面。
...
最后吐槽几句，辛辛苦苦做了几个月，老师不看个人劳动成果统一给钱，我真气炸了。
